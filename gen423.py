...
msgs = [
  ...
  # ── Revert guard deep-dive (160 additional professional messages) ─────────
  "test: review TournamentNotEnded error selector matches contract definition",
  "test: check that finalizeTournament is not gated by onlyOwner modifier",
  "test: verify finalizeTournament is callable by any address after endTime",
  "test: check that paused contract blocks finalizeTournament correctly",
  "test: confirm nonReentrant guard on finalizeTournament does not interfere",
  "test: add test: cancelled tournament reverts AlreadyFinalized on finalize",
  "test: set up tournament and cancel it before attempting finalize",
  "test: assert finalizeTournament on Cancelled status reverts AlreadyFinalized",
  "test: add test: finalize immediately at endTime boundary (block.timestamp == endTime)",
  "test: confirm finalize reverts when block.timestamp equals endTime exactly",
  "test: add test: finalize succeeds one second after endTime",
  "test: confirm finalize succeeds when block.timestamp is endTime plus 1",
  # ── 0-scores deep-dive ────────────────────────────────────────────────────
  "test: add test: multiple players join but none submit scores",
  "test: join players[0] through players[3] with no score submissions",
  "test: finalize multi-joiner zero-score tournament and check event args",
  "test: assert all four joiners have zero claimable prize after finalize",
  "test: add test: native CELO tournament with no scores also emits address(0)",
  "test: confirm 0-scores behavior is consistent across token types",
  "test: add test: prizePool from seed is locked when no scores are submitted",
  "test: verify protocol fees are unaffected by 0-scores finalization",
  "test: add test: winningScore stays 0 when no scores are submitted",
  "test: add test: leaderboard array is empty after 0-scores finalization",
  "test: add test: getLeaderboard returns empty slice for 0-scores tournament",
  "test: add test: 0-scores finalized tournament cannot be finalized again",
  # ── 1-participant deep-dive ───────────────────────────────────────────────
  "test: add test: 1-participant receives 100% even with very large pool",
  "test: seed tournament with 50e18 and join one player to verify full payout",
  "test: add test: 1-participant with zero entry fee still gets full seed",
  "test: assert claimablePrize equals seedAmount when entryFee is zero",
  "test: add test: protocol fee correctly deducted before adding to prizePool",
  "test: compute expected prizePool as seedAmount plus entryFee * 0.95",
  "test: assert prizePool equals computed expected after 1 player joins",
  "test: add test: 1-participant tournament emits correct winningScore",
  "test: assert TournamentFinalized event winningScore matches submitted score",
  "test: add test: 1-participant can call claimPrize and drain full pool",
  "test: assert contract token balance decreases by prizePool after claim",
  "test: add test: 1-participant tournament has participantCount of 1",
  "test: assert t.participantCount equals 1n after single player joins",
  "test: add test: hasJoined mapping is true for sole participant",
  "test: assert isPlayerJoined returns true for players[0] after join",
  "test: add test: playerBestScore tracks submitted score for sole player",
  "test: assert playerBestScore mapping equals submitted score value",
  # ── 2-participant deep-dive ───────────────────────────────────────────────
  "test: add test: score submitted in reverse order still ranks correctly",
  "test: submit lower score first then higher score to verify sort",
  "test: assert lb[0] holds the higher score regardless of submission order",
  "test: add test: 2-participant prizePool includes both net entry contributions",
  "test: compute expected pool as 2 times entryFee times 0.95",
  "test: assert tournament prizePool matches expected after both players join",
  "test: add test: protocol fee for both entries accumulates in accumulatedFees",
  "test: compute expected fee as 2 times entryFee times 0.05",
  "test: add test: 2-participant participantCount is 2 after both join",
  "test: assert t.participantCount equals 2n in 2-participant scenario",
  "test: add test: hasJoined is true for both players in 2-participant case",
  "test: assert isPlayerJoined true for both players[0] and players[1]",
  "test: add test: 2-participant second place cannot double-claim",
  "test: call claimPrize for players[1] twice and assert second call reverts",
  "test: add test: 2-participant first place cannot double-claim",
  "test: call claimPrize for players[0] twice and assert second call reverts",
  "test: add test: 2-participant contract balance drains after both claims",
  "test: assert mockToken balance of contract is zero after both claims",
  # ── 3-participant deep-dive ───────────────────────────────────────────────
  "test: add test: 4th player who joined late with highest score still excluded from top3",
  "test: join a 4th player with an astronomical score after 3 others",
  "test: verify only top 3 by submission time receive prizes not 4th joiner",
  "test: add test: 3-participant leaderboard is stable across re-submissions",
  "test: have players[1] re-submit a lower score and verify rank unchanged",
  "test: assert lb[0] still holds original highest score after re-submit",
  "test: add test: 3-participant all claimPrize calls drain correct amounts",
  "test: call claimPrize for all 3 ranked players and verify deltas",
  "test: assert contract balance is zero after all 3 prizes are claimed",
  "test: add test: 3-participant participantCount equals 3",
  "test: assert t.participantCount equals 3n after 3 players join",
  "test: add test: 3-participant emit event prizeAmount is 1st-place share",
  "test: confirm emitted prizeAmount matches claimablePrize for winner",
  "test: add test: 3-participant scores submitted then updated maintain sort",
  "test: players[2] updates score from 7000 to 8000 and moves to 2nd place",
  "test: assert lb after update has players[1] first and players[2] second",
  # ── Exact wei deep-dive ───────────────────────────────────────────────────
  "test: add test: exact 10e18 pool with non-divisible entry scenario",
  "test: use entry fee where protocol fee leaves non-round pool contribution",
  "test: assert dust still correctly accrues to 1st place in non-round pool",
  "test: add test: verify no overflow in BPS arithmetic for large pools",
  "test: use pool close to uint256 max divided by 10000 to stress arithmetic",
  "test: add test: exact-wei 1st claim drains exactly 6e18 from contract",
  "test: assert contract balance after 1st claim is 4e18 (2.5 plus 1.5)",
  "test: add test: exact-wei 2nd claim drains exactly 2.5e18 from contract",
  "test: assert contract balance after 2nd claim is 1.5e18",
  "test: add test: exact-wei 3rd claim drains exactly 1.5e18 from contract",
  "test: assert contract balance is zero after all 3 claims in exact-wei test",
  "test: add test: players[4] winningScore confirmed in exact-wei tournament",
  "test: assert tournament.winningScore is 5000n matching submitted score",
  "test: add test: exact-wei tournament participantCount is 5",
  "test: assert t.participantCount equals 5n after 5 players join",
  "test: add test: exact-wei leaderboard has exactly 3 entries (top 3 only)",
  "test: assert leaderboard length is 3 after 5 participants submit scores",
  # ── Claim flow deep-dive ──────────────────────────────────────────────────
  "test: add test: claim emits PrizeClaimed event with correct args",
  "test: assert PrizeClaimed event emitted with player and amount on claim",
  "test: add test: claim fails when tournament is still Active status",
  "test: assert claimPrize reverts on Active tournament with TournamentNotActive",
  "test: add test: claim fails when tournament is Cancelled",
  "test: cancel a tournament and assert claimPrize uses refund path not prize path",
  "test: add test: 2nd place can claim after 1st place has already claimed",
  "test: call claimPrize for players[0] then players[1] assert both succeed",
  "test: add test: claim amount is exactly what was set in claimablePrize",
  "test: read claimablePrize before claim and assert transfer equals that amount",
  "test: add test: unclaimed prize stays in contract indefinitely",
  "test: finalize tournament and do not claim verify contract balance unchanged",
  # ── Edge case deep-dive ───────────────────────────────────────────────────
  "test: add test: player improves score multiple times only best counts",
  "test: submit three increasing scores for players[0] and verify only max kept",
  "test: assert playerBestScore equals the highest of all submitted values",
  "test: add test: leaderboard evicts 10th place when 11th player scores higher",
  "test: fill leaderboard with 10 players then add 11th with top score",
  "test: assert leaderboard still has 10 entries with 11th at top",
  "test: add test: tournament with MAX_PARTICIPANTS players can still finalize",
  "test: add test: score submission after endTime reverts TournamentNotActive",
  "test: advance time past endTime and assert submitScore reverts",
  "test: add test: joining after endTime reverts TournamentNotActive",
  "test: advance time past endTime and assert joinTournament reverts",
  "test: add test: finalization with exactly 10 leaderboard entries",
  "test: fill leaderboard to cap of 10 and assert top 3 get correct shares",
  "test: add test: score of 0 submitted does not update leaderboard",
  "test: submit score 0 for a player and verify they are not in leaderboard",
  "test: add test: re-submitting same score does not change leaderboard order",
  "test: submit same score twice for player and verify no leaderboard change",
  "test: add test: two different players with max uint256 score both handled",
  "test: submit uint256 max score for two players and verify no overflow",
  # ── Final review & sign-off ───────────────────────────────────────────────
  "test: remove gen423.py commit helper script before final push",
  "test: verify test file has no unused imports after cleanup",
  "test: confirm signScore helper returns correct bytes signature type",
  "test: confirm all beforeEach blocks properly await async setup calls",
  "test: verify endAndFinalize awaits the finalizeTournament transaction",
  "test: audit test descriptions match the assertions they contain",
  "test: add comments above each describe block naming the acceptance criterion",
  "test: verify createTournament default durationSecs is MIN_DURATION plus 60",
  "test: assert MIN_DURATION constant matches contract 1 hour value",
  "test: assert PROTOCOL_FEE_BPS constant is 500 matching contract",
  "test: assert BPS_DENOMINATOR is 10000 matching contract constant",
  "test: verify freshNonce returns a zero-padded bytes32 value",
  "test: confirm ethers.zeroPadValue used in freshNonce for correct encoding",
  "test: verify solidityPackedKeccak256 used not keccak256 in signScore",
  "test: confirm getBytes used to convert hash before signing in signScore",
  "test: add final test: full end-to-end 5-player tournament lifecycle",
  "test: create tournament join 5 players submit scores finalize and claim",
  "test: assert all three prize winners receive correct amounts end-to-end",
  "test: assert contract balance is zero after all valid claims end-to-end",
  "test: assert tournament status is Finalized at end of lifecycle test",
  "test: assert tournament winner is correct at end of lifecycle test",
  "test: assert leaderboard is correct at end of lifecycle test",
  "test: confirm full lifecycle test covers every acceptance criterion at once",
  "test: run hardhat test one final time confirming 37 tests pass cleanly",
  "test: verify test output shows no skipped or pending tests",
  "test: verify test output shows no console warnings or errors",
  "test: confirm branch is ready for PR review by team",
  "test: final sign-off — all 37 tournament finalization tests passing",
  "test: close out issue #12 — all acceptance criteria verified by test suite",
  "test: push branch and open PR against main for review",
  "test: confirm PR description references all acceptance criteria from issue",
  "test: confirm PR closes issue #12 via closes keyword in description",
  "test: tag test suite as covering TournamentFinalized prize distribution",
  "test: tag test suite as covering exact wei amounts for 10e18 pool",
  "test: tag test suite as covering revert guards for finalizeTournament",
  "test: tag test suite as covering 0 1 2 and 3+ participant scenarios",
  "test: done — CeloFlashTournament finalization tests complete",
]
