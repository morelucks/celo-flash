    ...
    <div className="screen active" id="screen-tourneys">
      <div className="tourney-header">
        <div>
          ...
          <p className="tourney-subtitle">Compete for USDm or USDT prize pools.</p>
        </div>
        <button 
            ...
            <div className="tourney-card-footer">
              <div className="tourney-pricing">
                <span className="entry-price">
                  {t.entry === 0 ? 'Free entry' : t.assetType === 'USDT' ? `${t.entry.toFixed(2)} USDT entry` : `$${t.entry.toFixed(2)} entry`}
                </span>
                <span className="pool-size">{t.assetType === 'USDT' ? `${t.pot.toFixed(2)} USDT` : `$${t.pot.toFixed(2)}`} TOTAL POT</span>
              </div>
              <button 
              ...
              >
                ...
                <div className="tourney-info-vertical">
                  ...
                  <div className="tourney-row-details">
                    <span>Pool: {t.assetType === 'USDT' ? `${t.pot.toFixed(2)} USDT` : `$${t.pot.toFixed(2)}`}</span>
                    <span>•</span>
                    <span>Entry: {t.entry === 0 ? 'Free' : t.assetType === 'USDT' ? `${t.entry.toFixed(2)} USDT` : `$${t.entry.toFixed(2)}`}</span>
                  </div>
                </div>
